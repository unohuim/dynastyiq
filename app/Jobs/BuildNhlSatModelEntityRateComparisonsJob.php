...
class BuildNhlSatModelEntityRateComparisonsJob implements ShouldQueue, ShouldBeUnique
{
    ...
    {
        return 'nhl-sat-model-entity-rate-comparisons:' . $this->modelRunId;
    }
    ...
    {
        return [
            'nhl-sat-model-entity-rate-comparisons',
    ...
    public function handle(NhlSatModelEntityRateComparisonBuilder $builder): void
    {
        ...
        $jobs = array_map(
            fn (array $entity): BuildNhlSatModelEntityRateComparisonForEntityJob => new BuildNhlSatModelEntityRateComparisonForEntityJob(
        ...
        $run->forceFill([
            'metrics' => array_merge($run->metrics ?? [], [
                'rate_comparison_entities_queued' => count($jobs),
    ...
    public function failed(Throwable $exception): void
    {
        ...
        Log::error('NHL SAT model entity /60 comparisons job failed.', [
    ...
    private static function markFinishedForRun(int $modelRunId, bool $failed): void
    {
        ...
        $run->forceFill([
            ...
            'metrics' => array_merge($run->metrics ?? [], [
                'rate_comparisons_completed_at' => now()->toIso8601String(),
                'rate_comparison_rows' => self::comparisonCountsForRun($modelRunId),
                'rate_comparison_aggregate_rows' => self::aggregateCountsForRun($modelRunId),
            ]),
            'completed_at' => now(),
        ])->save();

        self::broadcastForRun($modelRunId, $failed ? 'rate-comparisons-failed' : 'rate-comparisons-completed');
    }
    ...
    private function markFailed(string $message): void
    {
        ...
        self::broadcastForRun($this->modelRunId, 'rate-comparisons-failed');
    }

    /**
     * @return array<string, int>
     */
    private static function comparisonCountsForRun(int $modelRunId): array
    {
        $counts = DB::table('nhl_sat_model_entity_rate_comparison_buckets')
     ...
     * @return array<string, int>
     */
    private static function aggregateCountsForRun(int $modelRunId): array
    {
        $counts = DB::table('nhl_sat_model_entity_rate_comparison_aggregates')
            ->where('model_run_id', $modelRunId)
            ->selectRaw('profile_type, COUNT(*) as rows')
            ->groupBy('profile_type')
            ->pluck('rows', 'profile_type')
    ...
    private static function broadcastForRun(int $modelRunId, string $reason): void
    {
        try {
            ...
            Log::warning('NHL SAT model /60 comparison broadcast failed.', [
