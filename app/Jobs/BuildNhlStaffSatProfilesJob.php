...
/**
 * Coordinates historical staff SAT profile jobs.
 */
class BuildNhlStaffSatProfilesJob implements ShouldQueue, ShouldBeUnique
{
    use Queueable;
    ...
    {
        return 'nhl-staff-sat-profiles:' . $this->sourceSeasonId . ':' . $this->gameType;
    }
    ...
    {
        $setup = $builder->prepareStaffBuild($this->sourceSeasonId, $this->gameType);
        $jobs = array_map(
            fn (array $assignment): BuildNhlStaffSatProfileForStaffJob => new BuildNhlStaffSatProfileForStaffJob(
                ...
                teamContext: $assignment['team_context']
            ),
            $setup['assignments']
        );

        if ($jobs === []) {
            Log::warning('No NHL staff SAT profile jobs were queued.', [
        ...
        Log::info('NHL staff SAT profile jobs queued.', [
            'source_season_id' => $setup['source_season_id'],
            'game_type' => $setup['game_type'],
            'goal_model_id' => $setup['goal_model_id'],
            'sog_model_id' => $setup['sog_model_id'],
            'staff_job_count' => count($jobs),
        ]);
    }

    public function failed(Throwable $exception): void
    {
        Log::error('NHL staff SAT profiles job failed.', [
