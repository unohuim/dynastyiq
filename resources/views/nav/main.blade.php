...
<div x-data="{
    ...
    <nav class="hidden sm:flex items-center justify-between bg-white border-b shadow px-6 py-2">
        <div class="flex items-center space-x-8">
            ...
            <a href="{{ route('stats.index') }}"
                class="text-lg font-semibold {{ request()->routeIs('stats.index') ? 'text-indigo-600' : 'text-gray-600 hover:text-indigo-500' }}">Stats</a>

            @auth
                <a class="text-lg font-semibold text-gray-600 hover:text-indigo-500">Perspectives</a>

                <a x-show="hasFantrax" href="{{ route('leagues.index') }}"
                    class="text-lg font-semibold {{ request()->routeIs('leagues.index') ? 'text-indigo-600' : 'text-gray-600 hover:text-indigo-500' }}">Leagues</a>

                <a href="{{ route('stats.units.index') }}"
                    class="text-lg font-semibold {{ request()->routeIs('stats.units.index') ? 'text-indigo-600' : 'text-gray-600 hover:text-indigo-500' }}">Line
                    Combos</a>
            @endauth
        </div>
        ...
        <div>
            ...
            @guest
                <a href="{{ route('discord.redirect') }}"
                    ...
                    <img src="{{ asset('images/Discord-Symbol-White.svg') }}" alt="Discord"
                        class="h-6 w-6">
        ...
        <ul class="flex items-center justify-between px-4 py-2 text-xs font-medium text-gray-300">
            ...
            <li class="flex-1 text-center">
                <a href="{{ route('stats.index') }}" class="flex flex-col items-center">
                    <svg class="h-6 w-6 mb-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                        stroke-width="1.5">
                        <path stroke-linecap="round" stroke-linejoin="round"
    ...
    @auth
        <div class="fixed inset-0 z-50 sm:hidden pointer-events-none">
            ...
            {{-- panel --}}
            <section x-show="leftOpen" x-transition:enter="transform ease-in-out duration-300"
                ...
                style="will-change: transform;" @keydown.escape.window="leftOpen=false" @click.away="leftOpen=false"
                ...
                <div class="flex items-center justify-between px-4 py-4 border-b border-white/10">
                    <div class="flex items-center gap-3">
                        <div
                            class="h-9 w-9 rounded-xl bg-indigo-500/20 flex items-center justify-center ring-1 ring-white/10">
                            <svg class="h-5 w-5 text-indigo-300" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                stroke-width="1.5">
                                <path stroke-linecap="round" stroke-linejoin="round"
                                    d="M3.75 6.75h16.5M3.75 12h16.5M3.75 17.25h16.5" />
                            </svg>
                ...
                <div class="h-full overflow-y-auto py-3">
                    @php
                        ...
                        <a href="'.$href.'"
                            ...
                            <svg class="ml-auto h-4 w-4 text-gray-400 group-hover:text-gray-300" viewBox="0 0 24 24"
