@php
    ...
    $desktopLink = 'inline-flex items-center px-1 pt-1 border-b-2 border-transparent text-sm font-medium leading-5 transition duration-150 ease-in-out text-lg font-semibold';
@endphp
...
<div
    ...
    x-init="
        ...
        window.addEventListener('integration:updated', (event) => {
        ...
        window.addEventListener('org:settings-updated', (event) => {
            const detail = event.detail || {};
            const value = detail.enabled ?? (detail.settings !== undefined && detail.settings !== null);
            hasCommunities = !!value;
        });
    "
    class="relative"
    x-cloak
>
    <nav class="relative z-[100] hidden md:flex items-center justify-between bg-white border-b shadow px-6 py-2">
        <div class="flex items-center space-x-8">
            <a
                ...
                class="{{ $desktopLink }} {{ request()->routeIs('welcome', 'dashboard') ? 'text-indigo-600 border-indigo-500' : 'text-gray-600 hover:text-indigo-500 hover:border-gray-300' }}"
            >Home</a>

            <a
                href="{{ route('games.index') }}"
                class="{{ $desktopLink }} {{ request()->routeIs('games.*') ? 'text-indigo-600 border-indigo-500' : 'text-gray-600 hover:text-indigo-500 hover:border-gray-300' }}"
            >Games</a>
            ...
            @auth
                <a
                    ...
                    class="{{ $desktopLink }} {{ request()->routeIs('leagues.index') ? 'text-indigo-600 border-indigo-500' : 'text-gray-600 hover:text-indigo-500 hover:border-gray-300' }}"
                >Leagues</a>

                <a
                    href="{{ route('stats.units.index') }}"
                    class="{{ $desktopLink }} {{ request()->routeIs('stats.units.index') ? 'text-indigo-600 border-indigo-500' : 'text-gray-600 hover:text-indigo-500 hover:border-gray-300' }}"
                >Line Combos</a>
                ...
                <a
                    ...
                    class="{{ $desktopLink }} {{ request()->routeIs('communities.index') ? 'text-indigo-600 border-indigo-500' : 'text-gray-600 hover:text-indigo-500 hover:border-gray-300' }}"
                >Communities</a>
            @endauth
        </div>
        ...
        <div>
            @auth
                <button type="button" @click="accountOpen = true" class="block focus:outline-none">
                    <img src="{{ $avatarUrl }}" alt="Open settings" class="h-9 w-9 rounded-full ring-2 ring-gray-200 object-cover" />
                </button>
            @endauth

            @guest
                <a href="{{ route('discord.redirect') }}" class="flex items-center space-x-2 px-3 py-2 bg-[#5865F2] text-white rounded-full hover:bg-[#4752C4]">
    ...
    <nav class="fixed inset-x-0 bottom-0 z-[100] bg-gray-900 text-gray-100 border-t shadow md:hidden">
        <ul class="flex items-center justify-between px-4 py-2 text-xs font-medium text-gray-300">
            ...
            <li class="flex-1 text-center">
                <a href="{{ route('games.index') }}" class="flex flex-col items-center {{ request()->routeIs('games.*') ? 'text-white' : '' }}">
                    <svg class="mb-0.5 h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 6.75h7.5m-9 3.75h10.5m-12 3.75h13.5m-12 3.75h10.5M6 3.75h12A2.25 2.25 0 0120.25 6v12A2.25 2.25 0 0118 20.25H6A2.25 2.25 0 013.75 18V6A2.25 2.25 0 016 3.75Z"/>
                    </svg>
                    Games
                </a>
            </li>
            ...
            <li class="flex-1 text-center">
                <a href="{{ route('stats.index') }}" class="flex flex-col items-center text-white">
            ...
            @auth
                ...
                <li class="flex-1 text-center">
                    <button type="button" @click="accountOpen = true" class="flex flex-col items-center w-full">
                        <img src="{{ $avatarUrl }}" alt="Your avatar" class="h-9 w-9 rounded-full ring-2 ring-gray-200 object-cover" />
    ...
    @auth
        <div class="fixed inset-0 z-50 md:hidden pointer-events-none">
            <div x-show="leftOpen" x-transition.opacity.duration.250ms @click="leftOpen=false" class="absolute inset-0 bg-black/40 pointer-events-auto"></div>
            ...
            <section
                ...
                aria-modal="true"
                role="dialog"
                aria-labelledby="mobile-left-drawer-title"
            >
                ...
                <div class="h-full overflow-y-auto py-3">
                    ...
                    <div class="px-2 space-y-2">
                        {!! $item(route('profile.show'), 'Profile', $ico('M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.5 20.25a8.25 8.25 0 0115 0')) !!}

                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="w-full group flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-red-500/10 text-red-300">
