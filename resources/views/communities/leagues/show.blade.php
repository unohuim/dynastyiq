...
<x-community-hub-layout
    ...
    <div
        x-data="{
            ...
            openRosterShare(team) {
                if (!team?.can_share_roster || !team?.share_links_url) return;
            ...
            loadRosterShareLinks() {
                if (!this.rosterShareTeam?.share_links_url || this.rosterShareLoading) return;

                this.rosterShareLoading = true;
                this.rosterShareError = '';

                fetch(this.rosterShareTeam.share_links_url, {
                    headers: {
                        'Accept': 'application/json',
                        'X-Requested-With': 'XMLHttpRequest'
                    }
                    ...
                    .catch((payload) => {
                        this.rosterShareError = payload?.message || 'Could not load share links.';
                    })
                    .finally(() => {
                        this.rosterShareLoading = false;
                    });
            },
            createRosterShareLink() {
                if (!this.rosterShareTeam?.share_link_create_url || this.rosterShareSaving) return;

                this.rosterShareSaving = true;
                this.rosterShareError = '';
                this.rosterShareMessage = '';

                fetch(this.rosterShareTeam.share_link_create_url, {
                    method: 'POST',
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type': 'application/json',
                    ...
                    .catch((payload) => {
                        this.rosterShareError = payload?.message || 'Could not create share link.';
                    })
                    .finally(() => {
                        this.rosterShareSaving = false;
                    });
            },
            updateRosterShareLink(link, values) {
                if (!this.rosterShareTeam?.share_links_url || !link?.id || this.rosterShareSaving) return;

                this.rosterShareSaving = true;
                this.rosterShareError = '';
                this.rosterShareMessage = '';

                fetch(`${this.rosterShareTeam.share_links_url}/${link.id}`, {
                    method: 'PUT',
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type': 'application/json',
                    ...
                    .catch((payload) => {
                        this.rosterShareError = payload?.message || 'Could not update share link.';
                    })
                    .finally(() => {
                        this.rosterShareSaving = false;
                    });
            },
            revokeRosterShareLink(link) {
                if (!this.rosterShareTeam?.share_links_url || !link?.id || this.rosterShareSaving) return;
                ...
                fetch(`${this.rosterShareTeam.share_links_url}/${link.id}`, {
                    ...
                    .catch((payload) => {
                        this.rosterShareError = payload?.message || 'Could not revoke share link.';
                    })
                    .finally(() => {
                        this.rosterShareSaving = false;
                    });
            ...
            copyRosterShareLink(link) {
                if (!link?.copy_enabled || !link?.url || !navigator.clipboard) return;
    ...
    >
                        ...
                        @foreach ($vm['teams'] as $team)
                            <li class="rounded-lg border border-slate-200 bg-slate-50/60 px-3 py-2">
                                <div class="flex items-center justify-between gap-3">
                                    ...
                                    @if (! empty($team['can_share_roster']))
                                        <button
                                            type="button"
                                            x-on:click="openRosterShare(@js($team))"
                                            class="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-500 shadow-sm transition hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                                            aria-label="Share roster for {{ $team['name'] }}"
                                        >
                                            <svg viewBox="0 0 20 20" fill="currentColor" class="h-4 w-4" aria-hidden="true">
        ...
        @if ($vm['header']['can_edit'])
        <x-ui.slide-over show="rosterShareOpen" close-action="rosterShareOpen = false" title-id="roster-share-title" max-width="max-w-lg">
            <div class="border-b border-slate-200 px-6 py-5">
                <div class="flex items-start justify-between gap-4">
                    <div class="min-w-0">
                        <h3 id="roster-share-title" class="text-base font-semibold text-slate-900">Roster Share</h3>
                        <p class="mt-1 truncate text-sm text-slate-500" x-text="rosterShareTeam?.name || 'Selected team'"></p>
                    </div>
                    <button
                        type="button"
                        x-on:click="rosterShareOpen = false"
                        aria-label="Close roster share"
                        class="rounded-md p-1.5 text-slate-500 hover:bg-slate-100 hover:text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                    >
                        <svg viewBox="0 0 20 20" fill="currentColor" class="h-4 w-4" aria-hidden="true">
                            <path fill-rule="evenodd" d="M5.22 5.22a.75.75 0 0 1 1.06 0L10 8.94l3.72-3.72a.75.75 0 1 1 1.06 1.06L11.06 10l3.72 3.72a.75.75 0 1 1-1.06 1.06L10 11.06l-3.72 3.72a.75.75 0 1 1-1.06-1.06L8.94 10 5.22 6.28a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd"/>
                        </svg>
            ...
            <div class="min-h-0 flex-1 overflow-y-auto px-6 py-5">
                ...
                <button
                    type="button"
                    x-on:click="createRosterShareLink()"
                    x-bind:disabled="rosterShareSaving || rosterShareLoading"
                    class="mb-5 inline-flex h-9 items-center gap-2 rounded-md bg-slate-900 px-3 text-sm font-semibold text-white hover:bg-slate-700 disabled:cursor-wait disabled:opacity-60"
                >
                    <svg viewBox="0 0 20 20" fill="currentColor" class="h-4 w-4" aria-hidden="true">
                ...
                <div x-show="rosterShareLoading" class="rounded-lg border border-slate-200 bg-slate-50 px-4 py-5 text-sm text-slate-500">
                    Loading share URLs...
                </div>

                <div x-show="!rosterShareLoading && rosterShareLinks.length === 0" class="rounded-lg border border-slate-200 bg-slate-50 px-4 py-5 text-sm text-slate-500">
                    No roster URLs have been created for this team.
                </div>
                        ...
                        <div class="rounded-lg border border-slate-200 bg-white px-4 py-3 shadow-sm">
                            <div class="flex items-center justify-between gap-4">
                                ...
                                <label class="inline-flex shrink-0 items-center gap-2 text-xs font-semibold text-slate-700">
                                    ...
                                    <button
                                        type="button"
                                        x-on:click="updateRosterShareLink(link, { is_public: !link.is_public })"
                                        x-bind:disabled="rosterShareSaving || link.revoked_at"
                                        x-bind:aria-pressed="link.is_public ? 'true' : 'false'"
                                        class="relative inline-flex h-5 w-9 items-center rounded-full transition disabled:cursor-not-allowed disabled:opacity-50"
                                        x-bind:class="link.is_public ? 'bg-indigo-600' : 'bg-slate-200'"
                                    >
                            ...
                            <div class="mt-3 flex items-center gap-2">
                                <input
                                    ...
                                    class="min-w-0 flex-1 rounded-md border-slate-200 bg-slate-50 text-xs text-slate-600 focus:border-indigo-500 focus:ring-indigo-500"
                                >
                                <button
                                    type="button"
                                    x-on:click="copyRosterShareLink(link)"
                                    x-bind:disabled="!link.copy_enabled"
                                    class="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40"
                                    aria-label="Copy public roster URL"
                                >
                                    <svg viewBox="0 0 20 20" fill="currentColor" class="h-4 w-4" aria-hidden="true">
                                ...
                                <button
                                    type="button"
                                    x-on:click="revokeRosterShareLink(link)"
                                    x-bind:disabled="rosterShareSaving || link.revoked_at"
                                    class="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-500 hover:border-rose-200 hover:bg-rose-50 hover:text-rose-700 disabled:cursor-not-allowed disabled:opacity-40"
                                    aria-label="Revoke public roster URL"
                                >
                                    <svg viewBox="0 0 20 20" fill="currentColor" class="h-4 w-4" aria-hidden="true">
