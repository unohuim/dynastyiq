<x-app-layout>
    @php
        $items = $leagues->map(function ($l) {
            return [
                'id'          => $l->id,
                'name'        => $l->name,
                'ext'         => $l->platform_league_id,
                'platform'    => $l->platform,
                'teams'       => $l->teams_count ?? null, // use withCount('teams') in controller if needed
    ...
    <div x-data="leaguesReadonly({ items: @js($items) })" class="px-4 py-6 max-w-7xl mx-auto">
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-5">
            <div>
                <h1 class="text-xl font-semibold text-gray-900">Leagues</h1>
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-[11px] inline-flex items-center gap-1 rounded-md bg-gray-100 px-1.5 py-0.5 text-gray-600"
                                      x-text="(item.platform||'')[0]?.toUpperCase() + (item.platform||'').slice(1)"></span>
                            </div>
                            ...
                            <div class="mt-3 flex flex-wrap gap-2">
                                <a :href="routes.index(item.id, 'trades')" class="group inline-flex items-center justify-center rounded-full border px-2.5 py-2 hover:bg-gray-50" title="Trades" aria-label="Trades">
                                    <svg class="w-4 h-4 text-gray-600 group-hover:text-gray-900" viewBox="0 0 24 24" fill="currentColor"><path d="M7 7h7a4 4 0 110 8H6l2.3 2.3a1 1 0 11-1.4 1.4L2.6 13l4.3-4.3a1 1 0 111.4 1.4L6 9h8a2 2 0 100-4H7a1 1 0 110-2z"/></svg>
                                </a>
                                <a :href="routes.index(item.id, 'rankings')" class="group inline-flex items-center justify-center rounded-full border px-2.5 py-2 hover:bg-gray-50" title="Rankings" aria-label="Rankings">
                                    <svg class="w-4 h-4 text-gray-600 group-hover:text-gray-900" viewBox="0 0 24 24" fill="currentColor"><path d="M5 3h4v18H5zM10 10h4v11h-4zM15 6h4v15h-4z"/></svg>
                                </a>
                                <a :href="routes.index(item.id, 'schedule')" class="group inline-flex items-center justify-center rounded-full border px-2.5 py-2 hover:bg-gray-50" title="Schedule" aria-label="Schedule">
                                    <svg class="w-4 h-4 text-gray-600 group-hover:text-gray-900" viewBox="0 0 24 24" fill="currentColor"><path d="M6 2a1 1 0 011 1v1h10V3a1 1 0 112 0v1h1a2 2 0 012 2v3H3V6a2 2 0 012-2h1V3a1 1 0 112 0v1zM3 20a2 2 0 002 2h14a2 2 0 002-2V10H3v10z"/></svg>
                                </a>
                                <a :href="routes.index(item.id, 'trends')" class="group inline-flex items-center justify-center rounded-full border px-2.5 py-2 hover:bg-gray-50" title="Trends" aria-label="Trends">
                                    <svg class="w-4 h-4 text-gray-600 group-hover:text-gray-900" viewBox="0 0 24 24" fill="currentColor"><path d="M3 17l6-6 4 4 7-7v5h2V5h-8v2h4l-5 5-4-4-8 8z"/></svg>
                                </a>
                            </div>
                        </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            ...
            Alpine.data('menu', () => ({
                ...
                openMenu() { this.open = true; this.$nextTick(() => this.place()); },
                place() {
