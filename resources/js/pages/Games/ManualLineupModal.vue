...
async function submit() {
  if (saving.value || (!text.value.trim() && !image.value)) return;
  saving.value = true;
  error.value = '';
  try {
  ...
  <Teleport to="body">
    <dialog ref="dialog" :aria-label="`Add ${team} lineup`" class="m-auto max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl border border-gray-200 bg-white p-6 shadow-xl backdrop:bg-gray-950/40" @cancel="saving ? $event.preventDefault() : emit('close')" @click="handleBackdropClick">
      <form class="space-y-4" @submit.prevent="submit">
        ...
        <textarea id="manual-lineup-text" v-model="text" autofocus :required="!image" maxlength="20000" rows="10" :disabled="saving" :placeholder="`Paste the ${team} lineup text or an image here…`" class="block w-full rounded-md border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500" @paste="pasteImage" />
        <div class="space-y-2">
        ...
        <button type="submit" :disabled="saving || (!text.trim() && !image)" class="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white transition-colors duration-150 hover:bg-indigo-500 disabled:opacity-50 motion-reduce:transition-none">{{ saving ? 'Processing…' : 'Submit' }}</button>
      </form>
    </dialog>
  </Teleport>
</template>
