...
const gameOrder = (game) => {
  if (game.game_state === 'FINAL') return 2;
  if (game.live_mode || (game.game_state && !['FUT', 'PRE'].includes(game.game_state))) return 0;
  return 1;
};
const puckDrop = (game) => {
  const timestamp = Date.parse(game.start_time_utc);
  return Number.isFinite(timestamp) ? timestamp : Number.MAX_SAFE_INTEGER;
};
const sortedGames = computed(() => [...payload.value.games].sort((a, b) =>
  gameOrder(a) - gameOrder(b) || puckDrop(a) - puckDrop(b)
));
