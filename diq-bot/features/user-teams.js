...
async function fetchUserTeams(targetDiscordId, viewerDiscordId) {
    ...
    if (viewerDiscordId)
        url.searchParams.set("viewer_discord_id", String(viewerDiscordId));
...
async function handle(interaction) {
    ...
    // Prefer guild displayName; fall back to globalName, then username, then "User"
    const display =
        (interaction.targetMember && interaction.targetMember.displayName) ||
    ...
    try {
        ...
        try {
            ...
            await interaction.editReply(
                "Could not DM you (are DMs disabled?)."
            );
        }
    } catch {
        await interaction.editReply(
