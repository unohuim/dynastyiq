...
async function ensureRole(guild) {
    ...
    console.log(
        `ℹ️ Creating role "${ROLE_NAME}" in ${guild.name} (${guild.id})`
    );
...
async function fetchAllMembers(guild) {
    try {
        ...
        console.warn(
            `⚠️ Could not fetch members for guild ${guild.id}: ${e?.message}`
        );
...
async function fetchConnectedIds(discordIds) {
    ...
    const list = Array.isArray(resp.data?.connected_ids)
        ? resp.data.connected_ids
        : [];
    ...
    if (set.size) {
        ...
        console.log(
            `🌐 sample connected_ids: [${sample}${set.size > 5 ? ", …" : ""}]`
        );
    }
    return set;
}
...
async function assignFantraxRole(client) {
    ...
    for (const guild of guilds) {
        ...
        try {
            ...
            console.warn(
                `⚠️ Could not ensure role in guild ${guild.id}: ${e?.message}`
            );
            continue;
        }
        ...
        const desired = new Set(
            [...connected].filter((id) => guildMemberIds.has(id))
        );
        ...
        for (const id of toAdd) {
            ...
            await limiter(() =>
                member.roles
                    .add(role, "DIQ Fantrax sync (add)")
                    .catch((e) =>
                        console.warn(
                            `⚠️ add failed ${guild.id}/${id}: ${e?.message}`
                        )
                    )
            );
        }
        ...
        for (const id of toRemove) {
            ...
            await limiter(() =>
                member.roles
                    .remove(role, "DIQ Fantrax sync (remove)")
                    .catch((e) =>
                        console.warn(
                            `⚠️ remove failed ${guild.id}/${id}: ${e?.message}`
                        )
                    )
            );
        }
