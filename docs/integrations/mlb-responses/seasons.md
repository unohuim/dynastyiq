...
| Path Template | `/seasons{all}` |
| Registry Key | `seasons` |
| Category | `Standings And Seasons` |
| Example URL | `https://statsapi.mlb.com/api/v1/seasons?sportId=1` |

## Parameters

Required parameter sets: `sportId`; or `divisionId`; or `leagueId`.
...
| `divisionId` | Provider-supported query parameter; verify accepted values before implementation. |
...
Document candidate MLB standings, season, and sport endpoints for season context and league structure.
