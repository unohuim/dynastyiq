...
| Path Template | `/stats/streaks` |
| Registry Key | `stats_streaks` |
| Category | `Stats And Leaders` |
| Example URL | `https://statsapi.mlb.com/api/v1/stats/streaks?sportId=1` |

## Parameters

Required parameter sets: `streakType`, `streakSpan`, `season`, `sportId`, `limit`.

| Parameter | Notes |
| --- | --- |
| `streakType` | Provider-supported query parameter; verify accepted values before implementation. |
| `streakSpan` | Provider-supported query parameter; verify accepted values before implementation. |
