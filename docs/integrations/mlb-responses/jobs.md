...
| Path Template | `/jobs` |
| Registry Key | `jobs` |
| Category | `Operations And Context` |
| Example URL | `https://statsapi.mlb.com/api/v1/jobs?sportId=1` |

## Parameters

Required parameter sets: `jobType`.

| Parameter | Notes |
| --- | --- |
| `jobType` | Provider-supported query parameter; verify accepted values before implementation. |
...
Document candidate MLB attendance and job-assignment endpoints for contextual or operational enrichment.
