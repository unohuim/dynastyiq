...
| Path Template | `/stats/leaders` |
| Registry Key | `stats_leaders` |
| Category | `Stats And Leaders` |
| Example URL | `https://statsapi.mlb.com/api/v1/stats/leaders?sportId=1` |

## Parameters

Required parameter sets: `leaderCategories`.

| Parameter | Notes |
| --- | --- |
| `leaderCategories` | Provider-supported query parameter; verify accepted values before implementation. |
| `playerPool` | Provider-supported query parameter; verify accepted values before implementation. |
| `leaderGameTypes` | Provider-supported query parameter; verify accepted values before implementation. |
| `statGroup` | Provider-supported query parameter; verify accepted values before implementation. |
...
| `statType` | Provider-supported query parameter; verify accepted values before implementation. |
