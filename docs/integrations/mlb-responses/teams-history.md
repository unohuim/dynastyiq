...
| Path Template | `/teams/history` |
| Registry Key | `teams_history` |
| Category | `Teams And Rosters` |
| Example URL | `https://statsapi.mlb.com/api/v1/teams/history` |

## Parameters

Required parameter sets: `teamIds`.

| Parameter | Notes |
| --- | --- |
| `teamIds` | Provider-supported query parameter; verify accepted values before implementation. |
| `startSeason` | Provider-supported query parameter; verify accepted values before implementation. |
| `endSeason` | Provider-supported query parameter; verify accepted values before implementation. |
