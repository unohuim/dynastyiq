...
| Path Template | `/game/{gamePk}/feed/color/diffPatch` |
| Registry Key | `game_color_diff` |
| Category | `Game Detail` |
| Example URL | `https://statsapi.mlb.com/api/v1/game/746001/feed/color/diffPatch` |

## Parameters

Required parameter sets: `startTimeCode`, `endTimeCode`.

| Parameter | Notes |
| --- | --- |
| `startTimecode` | Provider-supported query parameter; verify accepted values before implementation. |
| `endTimecode` | Provider-supported query parameter; verify accepted values before implementation. |

## Purpose

Document candidate MLB game-state, boxscore, live-feed, and event endpoints before DynastyIQ treats any field as import or validation authority.
