...
| Path Template | `/league` |
| Registry Key | `league` |
| Category | `League Reference` |
| Example URL | `https://statsapi.mlb.com/api/v1/league?sportId=1` |

## Parameters

Required parameter sets: `sportId`; or `leagueIds`.
...
| `seasons` | Provider-supported query parameter; verify accepted values before implementation. |
| `fields` | Provider-supported query parameter; verify accepted values before implementation. |

## Purpose

Document candidate MLB league, division, and conference reference endpoints.
