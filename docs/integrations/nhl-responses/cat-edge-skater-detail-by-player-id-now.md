# NHL Cat Edge Skater Detail by Player Id Now Response
...
| --- | --- |
| Method | `GET` |
| Path | `/cat/edge/skater-detail/{player-id}/now` |
...
| Example cURL | `curl -X GET "https://api-web.nhle.com/v1/cat/edge/skater-detail/8482116/now"` |
...
- `docs/api_responses/samples/nhlApi-cat-edge-skater-detail-by-player-id-now.txt`
...
- Top-level observed shape: player, seasonsWithEdgeStats, topShotSpeed, skatingSpeed, totalDistanceSkated, sogSummary, sogDetails, zoneTimeDetails
...
| `zoneTimeDetails` | object{9} | Keys: offensiveZonePctg, offensiveZonePercentile, offensiveZoneLeagueAvg, neutralZonePctg, neutralZonePercentile, neutralZoneLeagueAvg, defensiveZonePctg, defensiveZonePercentile, defensiveZoneLeagueAvg |
...
| `$.topShotSpeed.overlay` | empty array | [] | Observed provider field. | Candidate input only after source precedence is reviewed. | Do not treat as canonical without implementation-specific validation. |
