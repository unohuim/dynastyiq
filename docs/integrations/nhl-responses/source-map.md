...
| Game officials and head coaches | `/v1/gamecenter/{gameId}/right-rail` `gameInfo` | None. | Boxscore, PBP participants, or team roster payloads | Right-rail context is non-blocking enrichment. Referees, linesmen, and head coaches are persisted as game-scoped assignments, not as player identities. |
