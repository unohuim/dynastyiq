...
| `php artisan nhl:refresh-special-teams-splits --season=20252026 --queue` | Dispatches jobs with `--queue`; runs directly otherwise | Rebuild PBP-derived game-summary special-teams splits from already imported play-by-play rows. | Also supports `--game-id`, `--date-from`, and `--date-to`; run `nhl:sum` afterward. Use `--queue` for season/window backfills. |
...
| `php artisan fantrax:drafts:poll` | Dispatches jobs | Queue draft-state sync jobs for live Fantrax mirrored drafts. | Dispatches one draft sync job per due league. |
