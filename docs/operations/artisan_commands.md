...
| `php artisan nhl:game-context-sat-profiles --season=20252026 --game-type=2` | Dispatches jobs | Queue historical official and head-coach SAT profile buckets from shot-attempt facts and game context. | Requires scored xG and xSOG models for the same source season; also available from the NHL Shot Attempts admin `Refs & Coaches SAT` tab; supports `--only=all`, `--only=officials`, and `--only=staff`. Profile outputs expose exact SAT, selected parent bucket, parent SAT, prior weight SAT, shrinkage weight, and confidence wherever rates are inspected. |
...
| `php artisan nhl:refresh-game-context --season=20252026 --queue` | Dispatches jobs with `--queue`; runs directly otherwise | Refresh NHL right-rail game context such as referees, linesmen, and head coaches. | Also supports `--game-id`, `--date-from`, and `--date-to`; use `--queue` for season/window backfills so each game is its own job. |
