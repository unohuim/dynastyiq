...
- Update source, matching source, matched toggle, search, inbox selection, detail loading, and triage actions in place.
...
1. Audit current triage form, filter, search, row-selection, and action interactions for refresh paths.
2. Define the SearchField component contract:
...
3. Define the triage page JSON contract for inbox, detail, matching-source search, canonical search, and actions.
4. Add JSON responses/endpoints while preserving safe fallback behavior where practical.
5. Refactor `player-triage.js` to own page state, request cancellation, URL updates, and DOM rendering.
6. Replace Apply-dependent filter/search behavior with debounced or immediate AJAX changes.
7. Ensure row selection and detail pane loading do not refresh the page.
8. Ensure link, resolve, ignore, defer, and manual search actions update UI in place.
9. Add focused tests for JSON contracts, request behavior, URL state, and SearchField events.
10. Update canonical architecture docs and derived inventory once implementation details are approved.

## Test Plan

- Admin-only triage JSON endpoints reject unauthorized users.
- Triage filters return deterministic inbox payloads.
- Source/matching-source/matched changes update inbox payloads correctly.
