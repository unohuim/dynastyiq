...
Forecast season is separately configurable in run_config. Projection builders use
the explicit forecast season, otherwise the test season, otherwise the next season
after training. Comparisons still require a held-out test season.
...
- `database/migrations/2026_08_03_000001_add_sentry_failure_metadata_to_nhl_import_progress.php`

**Purpose:**
Centralize reads and writes to `nhl_import_progress` for import claim, status, dependency, stale-job behavior, run-aware game/stage uniqueness, rerun row seeding without historical run reassignment, and persisted failure diagnostics.
...
- `app/Services/NhlLineupPlayerResolver.php`
...
`NhlLineupPlayerResolver` is the shared local resolver for lineup names, abbreviations, aliases, and sweater references; lineup parsers and importers must not implement independent matching.
...
- `NhlLineupPlayerResolver::resolve()`
- `NhlLineupPlayerResolver::mentions()`
...
- `app/Models/PlatformLeague.php`
- `app/Models/PlatformLeaguePlayerStat.php`
...
- Platform league player stat rows
...
Game boxscore scheduling uses a per-game successful-refresh timestamp and persisted pregame and live intervals. The Vue Games drawer autosaves these settings and the start-before-puck-drop cutoff. Canonical rules: `docs/architecture/admin/AdminImportRegistry.yaml`.
...
- `docs/architecture/imports/NhlPublicGameLiveContext.yaml` — Enriches UTC-today public game cards with cached NHL state, live or final scores, and the same starting-goalie selection priority used by predictions.
- `resources/views/admin/nhl-shot-attempts/index.blade.php`
...
Reusable cross-sport publisher identity, per-team active scope, and time-varying metrics without implicit trust. The admin lineup-source API manages X identities and their many-to-many NHL team coverage without deleting evidence history. Multiple posts from one account remain one corroborating source. Authority: `docs/architecture/integrations/PublicEvidenceSources.yaml`.
...
Super admins can paste lineup text or a clipboard image, or upload a JPEG/PNG, through either team lineup badge on `/games`. The Vue modal posts to `POST /games/{nhlGameId}/lineup`, reusing OCR when an image is supplied, text parsing, verified identity checks, and observation/current-lineup persistence without X requests. Evidence is attributed to a per-user manual source; invalid submissions are rejected without replacing existing evidence. Valid submissions replace reported lineups with an attributed manual override, retained over subsequent automated evidence and used for prediction rosters without X searches. Cards update via AJAX.

The parent import dispatches only today's game/team jobs (America/Toronto) without inspecting lineup coverage. Admin and targeted refresh X timeline requests retrieve only today's posts; previously saved observations keep their existing eligibility. Each job evaluates its timing window and coverage before provider calls. X timeline scanning is capped at six five-post pages per account per discovery attempt, continuing round-robin through other sources until complete coverage or exhaustion. Retries restart attempt-local pagination.

NHL gamecenter boxscore rosters are checked only for game/teams without complete current lineup coverage; anticipated-lineup jobs use a dedicated two-worker `lineups` queue, and only paid X requests share the overlap lock so NHL checks may proceed concurrently. Only date-eligible current lineups passing NhlLineupPlayerResolver identity-and-slot verification suppress further discovery; unknown core players, duplicate identities, and incompatible positions remain unreported and eligible for correction, while unresolved F4/D3 players retain their explicit unresolved status and same-group peer fallback, while a complete official roster remains authoritative when no current lineup exists. Queue-level timeouts and exhausted attempts record terminal failures against the parent import so progress cannot remain working indefinitely. Otherwise, X discovery uses only stored team-scoped source timelines and never generic keyword search. It reads five posts per source in round-robin order and follows pagination until it finds a verified full single-post roster or exhausts the day-before-game boundary. All twelve forwards followed by six defensemen must appear in one roster-shaped block in the same post before player resolution begins. Partial posts remain audit-only; current truth never combines different observations for its forward and defense groups. Historical combined-post lineups cannot be reported or suppress discovery. Lineup extraction and persistence both use `NhlLineupPlayerResolver`; independent name matching is prohibited. The resolver uses canonical normalization, team context, unambiguous acronyms and surname prefixes, and recent team sweater evidence, and reprocessing may enrich unresolved identity fields without changing reported evidence. Same-team surname collisions use reported first initials and do not fall back across conflicting initials. Live terminal and Admin output contains only team, source handle, and five-post range; detailed decisions remain in local troubleshooting Markdown. Each local import removes prior generated lineup-audit Markdown while preserving its README and directories, then writes a new timestamped manifest, fresh team search files, complete timeline results, and per-post decisions. Official starter flags and ordered public G1 reports contribute starting-goalie evidence. Public `/games` pages show every scheduled matchup with independent team evidence statuses and current lineup detail while providing the route foundation for broader game information. Stored scores appear only after both participating teams have boxscore rows and the boxscore import stage is completed. Predictions may use an official or single-post reported lineup with all core skaters resolved; unresolved F4/D3 players may use the average of resolved peers in their exact group. Every resolved skater receives the same NHL, NHLe, or replacement-level game projection ladder regardless of lineup authority. Otherwise existing roster projection remains authoritative. Authority: `docs/architecture/imports/NhlAnticipatedLineups.yaml`.
