...
Immutable text-based anticipated-lineup evidence keyed by NHL game, canonical NHL team id, and post URL. Player rows repeat `team_id` and `team_abbrev` for direct inspection and retain normalized line slots, nullable reported `power_play_unit` and `penalty_kill_unit` values, and unresolved names alongside canonical player identifiers when resolution succeeds.
