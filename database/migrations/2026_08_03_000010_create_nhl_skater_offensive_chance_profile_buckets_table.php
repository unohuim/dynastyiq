...
return new class extends Migration
{
    ...
    {
        Schema::create('nhl_skater_offensive_chance_profile_buckets', function (Blueprint $table): void {
            $table->id();
            ...
            $table->unsignedInteger('source_sat_for')->default(0);
            $table->unsignedInteger('source_sog_for')->default(0);
            $table->unsignedInteger('source_goals_for')->default(0);
            $table->decimal('source_xgf', 10, 4)->nullable();
            $table->decimal('source_xsog', 10, 4)->nullable();
            $table->decimal('source_xgf_per_60', 10, 4)->nullable();
            $table->decimal('source_xsog_per_60', 10, 4)->nullable();
            ...
            $table->unique(
                ...
                'uq_nhl_skater_off_chance_profile_bucket'
            );
            $table->foreign('goal_expected_goals_model_id', 'fk_nhl_soc_goal_model')
                ->references('id')
                ->on('nhl_expected_goals_models')
                ->cascadeOnDelete();
            $table->foreign('shot_on_goal_expected_goals_model_id', 'fk_nhl_soc_sog_model')
                ->references('id')
                ->on('nhl_expected_goals_models')
                ->cascadeOnDelete();
            $table->index(['source_season_id', 'player_id'], 'ix_nhl_soc_season_player');
            $table->index(['source_season_id', 'team_abbrev'], 'ix_nhl_soc_season_team');
            $table->index(['source_season_id', 'fallback_level'], 'ix_nhl_soc_season_fallback');
        });
    }
    ...
    {
        Schema::dropIfExists('nhl_skater_offensive_chance_profile_buckets');
    }
};
