...
return new class extends Migration
{
    ...
    {
        Schema::create('nhl_skater_defensive_chance_profile_buckets', function (Blueprint $table): void {
            $table->id();
            ...
            $table->decimal('source_xga_per_60', 10, 4)->nullable();
            $table->decimal('source_xsoga_per_60', 10, 4)->nullable();
            ...
            $table->unique(
                ...
                'uq_nhl_skater_def_chance_profile_bucket'
            );
            $table->foreign('goal_expected_goals_model_id', 'fk_nhl_sdc_goal_model')
                ->references('id')
                ->on('nhl_expected_goals_models')
                ->cascadeOnDelete();
            $table->foreign('shot_on_goal_expected_goals_model_id', 'fk_nhl_sdc_sog_model')
                ->references('id')
                ->on('nhl_expected_goals_models')
                ->cascadeOnDelete();
            $table->index(['source_season_id', 'player_id'], 'ix_nhl_sdc_season_player');
            $table->index(['source_season_id', 'team_abbrev'], 'ix_nhl_sdc_season_team');
            $table->index(['source_season_id', 'fallback_level'], 'ix_nhl_sdc_season_fallback');
        });
    }
    ...
    {
        Schema::dropIfExists('nhl_skater_defensive_chance_profile_buckets');
    }
};
